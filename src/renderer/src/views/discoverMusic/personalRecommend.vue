...
const addClass = (index: number) => {
  ...
  if (activeIndex.value - 1 < 0) {
    if (index === bannerlists.value.length - 1) {
      return 'last'
    }
  }
  if (activeIndex.value === bannerlists.value.length - 1) {
    if (index === 0) {
      return 'next'
    }
  }
...
const handleChange = (type: string) => {
  if (type === 'next')
    activeIndex.value === bannerlists.value.length - 1 ? activeIndex.value = 0 : activeIndex.value++
  else if (type === 'last')
    activeIndex.value === 0 ? activeIndex.value = bannerlists.value.length - 1 : activeIndex.value--
}
let timer: number
onMounted(() => {
  timer = setInterval(() => {
...
<template>
  <div class="person-recommend">
    <div class="carousel" @mouseover="handleMouseOver" @mouseleave="handleMouseleave">
      ...
      <transition-group tag="div" class="carouselLists" ref="carouselRef">
